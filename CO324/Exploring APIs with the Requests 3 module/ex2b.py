...
list = github_superstars( orgnanization )  # print most star
...
with requests.Session() as session:
    ...
    # member list
    for name in git_memebrs.json():  # list of members
        git_memebrs_repo = session.get(
            f"https://api.github.com/users/{name['login']}/repos?-page=l&per_page=68")  # member repos
        ...
        for star in git_memebrs_repo.json():
            ...
            for x in list:
                if( star['name'] == x[1] ):
                    text = f"https://api.github.com/repos/{name['login']}/{star['name']}/subscribers"

                    sub = session.get( text )       # subscibe repo

print( list[0] )                # most star
